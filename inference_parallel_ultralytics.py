...
def predict(weights, img_queue, output_queue, stop_event):
    local_model = YOLO("best.pt").cpu()

    while not stop_event.is_set():
        try:
            img, x, y = img_queue.get(timeout=0.1)
            results = local_model.predict(img,  conf=0.25, iou=0.6, device='cpu')
...
inference_openvino("/home/gleb/learning/Detections/best_openvino_model/", "dataset/test_images/train_ZRI_2070_JPG.rf.826eb42eeed0ddf7d242a0bc0436f95a.jpg", "out_yolo_cpu_parallel.png")